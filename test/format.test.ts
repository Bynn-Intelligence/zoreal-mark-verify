...
describe('markers', () => {
  it('wraps and finds a Mark', () => {
    ...
    expect(s).toBe(`::ZOREAL-SIGNED:: I was there. ::ZOREAL-SIGNATURE:${ID}::`);
  ...
  it('finds several Marks and treats an inner opening marker as text', () => {
    const s = `${wrap('one', ID)} and ${wrap('two ::ZOREAL-SIGNED:: inner', ID)}`;
    const found = findMarks(s);
    expect(found.map((f) => f.text)).toEqual(['one', 'two ::ZOREAL-SIGNED:: inner']);
  });
  ...
  it('reports a truncated Mark and a bad id as broken, not as found', () => {
    expect(findMarks('::ZOREAL-SIGNED:: cut off by the platform')).toEqual([]);
    expect(findBrokenMarkers('::ZOREAL-SIGNED:: cut off by the platform')).toEqual([{ marker: 'signed', start: 0, reason: 'no_closing_marker' }]);
    expect(findMarks('::ZOREAL-SIGNED:: x ::ZOREAL-SIGNATURE:NOTANID::')).toEqual([]);
    expect(findBrokenMarkers('::ZOREAL-SIGNED:: x ::ZOREAL-SIGNATURE:NOTANID::')[0]?.reason).toBe('bad_id');
  });
